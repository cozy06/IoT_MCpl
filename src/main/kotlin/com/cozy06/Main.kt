import httplogic.HttpRequestHelper
...
//                        getIoTList(player)
    ...
    fun thingsclick(product_name: String, player: Player) {
        CoroutineScope(IO).launch {
            // do something
            val result = HttpRequestHelper().requestKtorIo_things(product_name)
            player.sendMessage(result)
        }
    }

    fun getIoTList(player: Player) {
        CoroutineScope(IO).launch {
            // do something
            val result = HttpRequestHelper().requestKtorIo_IoTList()
            val IoTList = result.split("|")
            for (i in IoTList.indices) {
                player.sendMessage(IoTList[i])
            }
        }
    }
}